...
def thread_check_button():
    ...
    while True:
        try:
            ...
            wlogging.log_exception("BTN_ERROR", "Error in button check loop", err)
...
def check_time ():
    ...
    try:
        ...
        wlogging.log_exception(LogMessage.ERR_WIFI_CONN.name, LogMessage.ERR_WIFI_CONN.value, err)
        return False
    try:
        ...
        wlogging.log_exception("TIME_CHG_ERR", f"Error updating time display for {current_time}", err)
...
def is_today_regional_holiday():
    ...
    try:
        ...
        wlogging.log_exception("HOLIDAY_ERR", "Error checking regional holiday", err)
        return False   
...
if __name__ == "__main__":
    ...
    while True:
        try:
            ...
            wlogging.log_exception("MAIN_LOOP_ERR", "Unhandled error in main loop", err)
        time.sleep(1)
